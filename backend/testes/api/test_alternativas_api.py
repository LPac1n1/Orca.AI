...
from apoio_api import COLETA, LEITE, LOJAS, PAPEL, NavegadorFalso, abrir_navegador_falso, cliente_cnpj, pdf_valido
...
ALTERNATIVAS = {  # domínio: [(endereço, título, preço)] — o que a busca de cada loja do trio mostra
    "gimba.com.br": [("moca", "Leite Condensado Moça 395g", 1100), ("italac", "Leite Condensado Italac 395g", 950),
                     ("pira", "Leite Condensado Piracanjuba 395g", 1080)],
    "tendaatacado.com.br": [("pira", "Leite Condensado Piracanjuba 395g", 1000),
                            ("italac", "Leite Condensado Italac 395g", 1000)],
    "lepok.com.br": [("italac", "Leite condensado Italac 395 g", 1020), ("pira", "Leite Condensado Piracanjuba 395g", 990)],
}
...
class NavegadorComBusca(NavegadorFalso):
    def __init__(self):
        super().__init__()
...
def _lote_com_leite_acima_da_media(api, app) -> dict:
    """O cenário dos testes da API: a Gimba é a escolhida e o leite passa da média (R$ 11,00 > R$ 10,50)."""
...
def test_saida_1_com_busca_de_ponta_a_ponta(ambiente):
    ...
    assert [l["busca"] for l in r["lojas"]] == ["Gimba (Supricorp Suprimentos)", "Tenda Atacado", "Lepok"]  # o trio
    ...
    usada = _ok(api.post(f"/api/itens/{ids['leite']}/usar-alternativa", json={
        "tarefa_id": tarefa["id"], "indice": 0, "descricao": "Leite condensado 395g", "marca": "Italac",
        "justificativa": "Saída 1 (D-23): Moça acima da média na Gimba"}), 201)
    novo = usada["item"]
    ...
    assert next(l for l in lote["lojas"] if l["posicao"] == 1)["cnpj"] == LOJAS["gimba.com.br"][0]  # continua a escolhida
