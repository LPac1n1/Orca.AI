...
| D-75 | **Código de barras primeiro (25/09/2026, escolha do usuário no piloto):** a descrição tem muitas variações; o código de barras é o mesmo em todas as lojas. Por isso a descrição serve só para achar o **primeiro** produto: ao cadastrar um item sem código de barras, o sistema pesquisa as lojas com busca automática que vendem o tipo do item (D-74) e mostra uma **vitrine** (foto, nome, prévia do preço e loja; só produtos do mesmo tipo e sem atributo contrário). A pessoa clica em **"É este"**: a página é capturada como prova, fica 🟢 por decisão dela ("escolhido na vitrine") e vira o **produto de referência** (D-71); o código de barras dela passa a guiar as buscas nas outras lojas (busca pelo código onde a loja aceita, 🟢 automático pelo mesmo código). Se a página não mostrar o preço, a escolha não vira referência e o sistema avisa. Nada é gravado só por abrir a vitrine. A vitrine também fica no item ("escolher o produto"), na tabela da pesquisa e no "Fechar o lote" (itens que faltam sem referência). Ensaio real: suco de uva Del Valle 1L — a vitrine mostrou o suco certo na Sonda; escolhido, o "Fechar o lote" achou o mesmo código no Atacadão (em falta), que a busca pela descrição não trazia. | Usuário |
