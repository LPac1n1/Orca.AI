...
│       ├── fluxo/             liga as etapas a partir do banco: estado, seleção, vagas, teto, painel, dossiê, fechamento do lote
...
- **Configuração:** cada loja do catálogo pode ter `busca: {modo, url, produto, seletor}`. `api_vtex` = API pública de catálogo (texto e código de barras); `api_vtex_is` = busca pública das lojas VTEX novas (texto e código de barras); `api_woocommerce` = API pública de produtos das lojas WordPress; `pagina` = página de busca do site, aberta pelo navegador sem janela, de onde se leem os links de produto (`produto` é o pedaço do endereço que os identifica; `seletor`, a lista de resultados — menus e "sugestões" fora dela não são lidos) e o texto do cartão de cada um; `assistida` = a janela abre na busca e a pessoa escolhe (lojas que recusam programas ou cujo robots.txt proíbe a busca). Levantamentos: 24/09/2026 e 25/09/2026 (docs/07 §5 e §6).
- **Escolha:** cada candidato passa pela mesma correspondência dos produtos (sem código de barras, pelo título); 🔴 nunca é capturado; **produto de outro tipo também não** — o título precisa começar pelo tipo do item (a primeira palavra da descrição, sem contar a marca) ou ter pelo menos 30% das palavras em comum (piloto de 25/09/2026: a busca trouxe "Louro em folhas" para um perfurador e "Grampos para grampeador" para um grampeador); entre os 🟡, ganha o mais parecido — palavras em comum sobre todas as palavras do item e do título, para que variações ("reciclado", "eco") percam para o produto comum. Se o primeiro não for 🟢 nem bem parecido, faz uma segunda busca sem as medidas e, se ainda preciso, uma terceira só com o tipo e a marca ("Caneta Bic"), para as buscas que exigem todas as palavras.
- **Prova:** a página do produto escolhido é capturada e registrada como ao colar o link (preço à vista, CNPJ consultado, correspondência pela página). "Não encontrado" é registrado com a página da busca. Se a página não traz o preço nos dados (WooCommerce, produto com variações), vale o preço que a busca mostrou **só se ele estiver escrito na página capturada** (aviso na observação; princípio 5).
- **Endereço da busca:** `{termo}` (texto) ou `{termo_slug}` (com hífens, ex.: Lepok `/busca/papel-a4`).
- **Código de barras:** as buscas por API (VTEX) e as páginas de busca marcadas com `ean: true` (Kalunga, conferido em 25/09/2026) procuram primeiro pelo código de barras do item ou do produto de referência (D-71); sem resultado, pelo texto. Essas lojas são pesquisadas por último, para aproveitar os códigos achados nas outras — inclusive os de páginas 🟡, só como pista de busca (quem confirma continua sendo a comparação ou a pessoa); se a busca pelo código não traz nada bom, a busca pelo texto roda do mesmo jeito.
- **Em falta:** página sem preço que diz "indisponível", "esgotado" ou "avise-me" fica registrada como produto em falta na loja (sem mandar para a janela).
- **Tempo:** a página de busca tem limite de 25 s; loja que não responde é pulada. Três buscas seguidas que trazem produtos, mas nenhum do tipo do item (de qualquer marca), indicam que a busca da loja mudou (como a da Lepok): os outros itens nela são poupados, com o motivo no resumo. Busca sem nenhum resultado não conta: é só a loja sem aquele item.
...
### 5.1.1a O que cada loja vende (D-74)

`orca.busca.classificacao` (sem rede): os tipos de loja e seus nomes, os produtos de teste de cada tipo e a regra `vende_o_tipo` (a busca pelo produto típico trouxe pelo menos 2 produtos do tipo — o mesmo filtro de "mesmo tipo" da busca). Tarefa `classificar_loja` (`orca.tarefas.lojas`): testa a busca da loja e grava `categorias`, `categorias_origem: automatica` e `categorias_em` numa nova versão do catálogo da OSC (autor `sistema:lojas`). É enfileirada sozinha quando a OSC salva uma loja com busca automática sem tipo, ou pelo botão "descobrir o que vende". `orca.fluxo.catalogos.categorias_aprendidas` junta, por domínio, os tipos dos itens com página 🟢 nas pesquisas da OSC. A rota `/api/lotes/{id}/lojas-de-busca` diz, para cada loja, a situação em relação ao lote (`todas`, `parte`, `nenhuma`, `desconhecida`, `itens_sem_categoria`), o que ela vende e o que foi aprendido.
...
### 5.1.1d Fechar o lote e produto de referência (piloto da Fase 2; D-71 a D-73, D-75)

- **Produto de referência:** decisão `produto_referencia` (tabela `decisao`, alvo = item, valor = observação), gravada quando a pessoa confirma a primeira página do item como 🟢 ou escolhe outra ("usar como referência", rota `/api/itens/{id}/referencia`). Vale enquanto a decisão vigente daquela página for 🟢 de uma pessoa (`orca.banco.referencia_do_item`). `corresponder` usa `orca.correspondencia.comparar_com_referencia` quando há referência; quando ela muda, `orca.fluxo.recomparar_item` compara de novo as páginas não decididas por uma pessoa (autor `sistema:referencia`), gravando só o que mudou.
- **Fotos:** `orca.coleta.imagem_da_pagina` lê a foto dos dados do produto (JSON-LD `image`) ou do `og:image`; fica em `dados_brutos.imagem` da observação e aparece na revisão (cada oferta e a referência do item). A interface carrega a foto do site da loja, sem mandar o endereço de origem (`referrerPolicy=no-referrer`).
- **Vitrine do item (D-75):** tarefa `vitrine_do_item` (`orca.tarefas.vitrine`), rota `POST /api/itens/{id}/vitrine`: pesquisa o item nas lojas com busca automática que vendem o tipo dele (`lojas_pelo_que_vendem`), com as mesmas buscas do lote (`_candidatos`: código de barras, termo, termo curto, termo mínimo) e a mesma triagem (`avaliar_candidatos`: sem 🔴 nem outro tipo), e devolve até 8 produtos por loja (endereço, título, marca, prévia do preço, foto, código de barras). Não grava nada. `POST /api/itens/{id}/escolher-produto` (`{url, justificativa?}`) enfileira `coletar_item` com `escolha_da_pessoa`: depois do registro, a correspondência 🟢 (`decidir_correspondencia`, motivo "escolhido na vitrine") e a decisão `produto_referencia` são gravadas em nome de quem pediu a tarefa, e `recomparar_item` compara de novo as outras páginas. Página sem preço: sem referência, com aviso.
- **Fechar o lote:** tarefa `fechar_lote` (`orca.tarefas.fechamento`), rota `/api/lotes/{id}/fechar`: pesquisa todos os itens (`pesquisar_lote`, a mesma da busca do lote), calcula o que falta (`orca.fluxo.fechamento`: lojas da mais completa para a menos — uma loja "tem" o item quando há página dele não recusada; itens sem referência com página 🟡 ou 🟢 a confirmar) e, para até 5 itens que faltam nas 3 lojas mais completas **com busca automática** (uma loja só com janela, como a Gimba, pode ser a mais completa, mas não dá para pesquisar nela), procura o item sem a marca nessas lojas e lista os produtos de outra marca que aparecem nas 3 (`produtos_nas_lojas`), do menor preço somado da prévia para o maior. Com a chave da SerpApi (opcional, D-70), para cada item que falta e tem código de barras (do item ou da referência), a tarefa também procura esse código no Google e lista até 5 outras lojas com a página do mesmo produto (uma por loja; lojas que já têm página não recusada do item ficam de fora) — só aponta: a pessoa escolhe o que ler, e a página com o mesmo código vira 🟢 pela referência (C2, D-67). A rota `/api/lotes/{id}/fechamento` devolve a situação ao vivo e as sugestões da última tarefa; `usar-alternativa` aceita as sugestões do fechamento (item do mesmo lote).

### 5.1.2 Vagas (Fase 2, etapa 11; D-69)

**Google Vagas (D-69 revista):** `orca.busca.serpapi.buscar_vagas_google` (SerpApi, `engine=google_jobs`) e tarefa `descobrir_vagas` (`orca.tarefas.opcionais`), rota `/api/cargos/{id}/vagas-google`. Para cada vaga, o link de candidatura de uma plataforma com `abrir_vaga: sistema` é capturado pelo sistema (até 6 por busca, as que mostram salário primeiro; as páginas já pesquisadas não são repetidas), e o registro é o de sempre (`registrar_cargo`, salário e CNPJ lidos da página, aviso "achada no Google Vagas"); as outras ficam listadas para a pessoa abrir (Indeed e LinkedIn na janela).
