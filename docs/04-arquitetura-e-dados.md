...
- **Fechar o lote:** tarefa `fechar_lote` (`orca.tarefas.fechamento`), rota `/api/lotes/{id}/fechar`: pesquisa todos os itens (`pesquisar_lote`, a mesma da busca do lote), calcula o que falta (`orca.fluxo.fechamento`: lojas da mais completa para a menos — uma loja "tem" o item quando há página dele não recusada; itens sem referência com página 🟡 ou 🟢 a confirmar) e, para até 5 itens que faltam nas 3 lojas mais completas, procura o item sem a marca nessas lojas e lista os produtos de outra marca que aparecem nas 3 (`produtos_nas_lojas`), do menor preço somado da prévia para o maior. A rota `/api/lotes/{id}/fechamento` devolve a situação ao vivo e as sugestões da última tarefa; `usar-alternativa` aceita as sugestões do fechamento (item do mesmo lote).
