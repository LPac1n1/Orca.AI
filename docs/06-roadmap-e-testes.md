...
- [x] **Conjunto de referência de correspondência:** 364 pares de produtos reais rotulados (60 mesmo / 304 diferente), em `backend/orca/correspondencia/referencia/pares_referencia.csv` (vai junto com o programa: o sistema roda o teste antes de salvar mudanças no vocabulário) — 10 do levantamento, o restante montado na etapa 7 a partir da API pública do Atacadão e do Open Food Facts (como foi feito: `LEIAME.md` na mesma pasta).
...
- Captura assistida (C4) nos conectores de vagas, inclusive LinkedIn (a janela em que o usuário navega já existe desde a etapa 9.4); link de loja achado pelo código de barras num buscador (C2, D-67); aviso antes de quantas capturas assistidas o orçamento vai exigir.
...
9. API e interface, em quatro partes (cada uma com a sua aprovação):
   ...
   4. ✅ Captura assistida (D-67): três jeitos de ler uma página — o sistema lê sozinho; janela do navegador em que o usuário navega e clica em “Capturar agora” (com “Cancelar”), numa fila própria que não trava as outras tarefas; PDF salvo e enviado pelo usuário, com a conferência do endereço, do nome e do valor no PDF. Comprovantes da Receita pela interface (o usuário resolve a verificação; o sistema salva sozinho). Catálogos da OSC editáveis na interface — vocabulário, categorias e lojas —, gravados como camada com versão por cima do catálogo do sistema (só a diferença); pares de exemplo que nascem das decisões, de códigos de barras iguais em lojas diferentes ou escritos à mão; toda mudança no vocabulário passa pelo teste de correspondência (os 364 pares do sistema + os da OSC) e é recusada se algum par de produtos diferentes virar 🟢 (D-64 a D-66). Regras do projeto e de cada orçamento editáveis na interface, com versão e origem de cada regra.
...
### Piloto da Fase 2, rodada 1 (25/09/2026)
A busca de um lote real de escritório (9 itens, 5 lojas) veio sem nenhum 🟢, com vários 🟡 e muitas lacunas. Diagnóstico nos dados: (1) a busca da Tenda devolve "sugestões" sem relação quando não acha o produto, e elas eram capturadas como 🟡 (louro, acelga, ventilador); (2) o robots.txt da Gimba e da Tenda proíbe a busca por programas; (3) os produtos certos da Gimba ficaram 🟡 porque o item não diz tudo (gramatura, quantidade) e a página tem palavras a mais; (4) numa loja sem um item, a busca parava (D-68). Corrigido: Gimba e Tenda com janela (D-67); só produto do mesmo tipo vira candidato; leitura só da lista de resultados; terceira busca com tipo e marca; 8 lojas novas (docs/07 §6). Depois, com as escolhas do usuário (25/09/2026): a busca segue nos outros itens quando falta um (D-68 revista); produto de referência (D-71); botão "Fechar o lote", com os produtos a confirmar e a sugestão de outra marca que existe nas 3 lojas (D-72); fotos lado a lado (D-73); Google Vagas pela SerpApi (D-69 revista). Testes de ponta a ponta com lojas simuladas: confirmar o papel de 75 g numa loja deixa as outras 🟢 pelo código de barras e o de 90 g 🔴; o grampeador que só existe numa loja ganha a sugestão de outra marca que existe nas 3, e a troca deixa as 3 lojas completas.

### Piloto da Fase 2, rodada 2 (25/09/2026)
"Fechar o lote" no lote real de escritório (8 lojas) levou 43 minutos e não trouxe nenhum produto certo. Diagnóstico nos dados: (1) a Lepok mudou a busca — o endereço antigo passou a mostrar os mais vendidos para qualquer termo; (2) a Kalunga vende marcas próprias (Spiral, Easy Office): Bacchi, Eagle e Goller não existem lá (🔴 marca diferente, certo); (3) na Pedagógica e na Papel Mais Cia a página não traz o preço nos dados; (4) as sugestões de outra marca não foram procuradas porque a loja mais completa (Gimba, da busca antiga) só abre na janela; (5) duas lojas travaram (limite de 45 s). Corrigido: busca nova da Lepok (ensaio real: clips Bacchi 2/0 com 100, caixa de canetas Bic Cristal com 50 e grampo Bacchi 26/6 com 5000 achados); preço da busca quando está escrito na página; sugestões procuradas nas lojas com busca automática; limite de 25 s na página de busca; loja cuja busca traz só produtos sem relação em 3 itens seguidos é poupada (a primeira versão dessa regra também parava a loja que só não tinha os itens mais raros, e foi corrigida). Rodada seguinte: com a Lepok nova, 7 dos 9 itens achados. O usuário pediu que o sistema separe as lojas pelo que vendem e classifique sozinho as novas (D-74). Depois: busca pelo código de barras nas lojas que aceitam (Kalunga) e, com a SerpApi, outras lojas com o mesmo código de barras no "Fechar o lote" (C2).

### Piloto da Fase 2, rodada 3 (25/09/2026)
Projeto novo, do zero, com um item ("Suco de uva Del Valle 1L"). A Sonda achou o suco certo (🟡 por causa do "100%"), o Extra também (na janela, 🟢), o Atacadão tinha o suco em falta e o Oba não vende. O Carrefour recusou até a janela com a pessoa navegando. Corrigido: o código de barras das páginas 🟡 passa a ser usado como pista de busca nas outras lojas; produto em falta é dito como tal; Carrefour só por PDF. O usuário observou que buscar pela descrição tem muitas variáveis e traz poucos resultados. Resposta (D-75, "código de barras primeiro"): vitrine no cadastro do item — o usuário escolhe o produto certo uma vez, e o código de barras dele guia as outras buscas. Teste de ponta a ponta com lojas simuladas (`testes/api/test_vitrine_api.py`): a vitrine mostra o suco da Sonda com a foto; escolhido, ele vira referência 🟢 com o código de barras, e a busca do lote acha o mesmo suco no Oba e no Atacadão só pelo código, sem busca pela descrição. Ensaio real: a vitrine trouxe 5 sucos da Sonda (o certo em primeiro); o Atacadão só tinha o de 200 ml (fora da vitrine, tamanho diferente) pela descrição, mas pelo código de barras achou o de 1 L (em falta).
...
12. ✅ Validade e avisos (D-12): quadro "Pesquisas vencidas ou vencendo" na aba de pesquisa, com "Pesquisar de novo" em cada uma e em todas (a mesma página; janela de novo se foi capturada na janela; PDF enviado só a pessoa refaz); aviso no Painel.
13. ✅ Aprendizado (D-65): aba "Sugestões" em Catálogos. Dos pares que o sistema ainda não acerta (vindos das decisões, do código de barras ou escritos à mão) saem sugestões — valor novo num grupo, sinônimo de um valor, grupo novo de valores que se excluem, ou atributo que a categoria passa a conferir —, só em casos claros (uma palavra de cada lado); cada uma é conferida no teste de correspondência (todos os pares) e só é aplicada pela pessoa, e nunca se cria um 🟢 errado novo.
14. ✅ Saídas 1 e 2 automáticas: no item acima da média, "procurar outra marca" pesquisa o item sem a marca nas 3 lojas do trio, junta os produtos que aparecem nelas e faz a conta de sempre com os preços da prévia; a pessoa escolhe, o produto é trocado (o antigo fica no histórico) e as 3 páginas vão para a fila como prova. Na Saída 2, o atalho "Pesquisar outras lojas" abre a busca do lote. Teste de ponta a ponta do T-04 pela API, com lojas simuladas.
15. ✅ Opcionais (D-70): tela "Opcionais" com as chaves guardadas no Gerenciador de Credenciais do Windows. Buscador (SerpApi grátis, C2): "Procurar pela internet" na janela de pesquisa do lote mostra páginas parecidas em outras lojas, e a pessoa escolhe quais o sistema lê. IA grátis (Gemini ou modelo local): "Pedir à IA para conferir" os 🟡; ela só rebaixa para 🔴, com motivo. Tudo desligado por padrão; testes com os serviços simulados, sem chave de verdade.
