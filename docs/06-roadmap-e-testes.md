...
9. API e interface, em quatro partes (cada uma com a sua aprovação):
   ...
   2. API local (FastAPI) e fila de tarefas (coleta, consulta de CNPJ, captura, exportação).
   3. Telas principais: projetos, orçamentos, itens e cargos, colar links, matriz de revisão, decisões, otimização, painel e documentos.
